#include "app_container_generator.h"
...
// CUSTOM
#include "container_stack.h"    // container_stack_*
#include "rtb_builder.h"        // RTB_*

/*      STATIC VARIABLES & CONSTANTS                                */

// Button to press.
#define                 CRE_BUTTON_IDX  BSP_BUTTON_0
#define                 DES_BUTTON_IDX  BSP_BUTTON_1
...
static void AppContainerGenerator_MsgHandler(container_t* container,
                                             msg_t* msg);
...
void AppContainerGenerator_Init(void)
{
    ...
    s_app_instance = Luos_CreateContainer(AppContainerGenerator_MsgHandler,
                                          APP_CONTAINER_GENERATOR_TYPE,
                                          APP_CONTAINER_GENERATOR_ALIAS,
                                          revision);
}
...
static void init_button(void)
{
    ...
    ret_code_t err_code = app_button_init(buttons, 2, BTN_DTX_DELAY);
...
static void button_evt_handler(uint8_t btn_idx, uint8_t event)
{
    ...
    if (btn_idx == CRE_BUTTON_IDX)
    {
        container_stack_push();
    }
    else if (btn_idx == DES_BUTTON_IDX)
    {
        container_stack_pop();
    }
